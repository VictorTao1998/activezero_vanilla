...
parser.add_argument('--output', type=str, default='../2_4_sequence', help='Path to output folder')
...
def test(transformer_model, psmnet_model, val_loader, logger, log_dir):
    ...
    for iteration, data in enumerate(tqdm(val_loader)):
        ...
        with torch.no_grad():
            pred_disp, pred_conf = psmnet_model(img_L, img_R, img_L_transformed, img_R_transformed)
        ...
        # TO BE deleted
        pred_conf_np = pred_conf.squeeze(0).squeeze(0).detach().cpu().numpy()  # in m, [H, W]
        # pred_depth_np = (pred_depth_np * 1000.0).astype(np.uint16)
        # pred_depth_np = pred_depth_np[:, :]
        ...
        cv2.imwrite(os.path.join(log_dir, 'pred_depth', f'conf_{iteration:04}.png'), pred_conf_np)
...
def main():
    ...
    val_loader = get_seq_loader('/code/supp_video/3/', idx_s=0, idx_e=204, debug=args.debug, sub=10)
