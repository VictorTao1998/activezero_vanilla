...
from nets.psmnet_confidence_old import PSMNet
...
def test(psmnet_model, val_loader, logger, log_dir):
    ...
    for iteration, data in enumerate(tqdm(val_loader)):
        ...
        # TODO
        img_L = F.interpolate(
            img_L, (540, 960), mode="nearest", recompute_scale_factor=False
        )
        img_R = F.interpolate(
            img_R, (540, 960), mode="nearest", recompute_scale_factor=False
        )
