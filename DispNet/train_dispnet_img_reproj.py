...
from utils.util import setup_logger, weights_init, adjust_learning_rate, \
...
def train(dispnet_model, dispnet_optimizer, TrainImgLoader, ValImgLoader):
    for epoch_idx in range(cfg.SOLVER.EPOCHS):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            adjust_learning_rate(dispnet_optimizer, global_step, cfg.SOLVER.LR_CASCADE, cfg.SOLVER.LR_STEPS)

            do_summary = global_step % args.summary_freq == 0
            # Train one sample
            scalar_outputs_dispnet, img_outputs_dispnet, img_output_reproj = \
            ...
            if (not is_distributed) or (dist.get_rank() == 0):
                ...
                if do_summary:
                    ...
                    save_images(summary_writer, 'train_dispnet', img_outputs_dispnet, global_step)
                    # Update PSMNet losses
                    scalar_outputs_dispnet.update({'lr': dispnet_optimizer.param_groups[0]['lr']})
                    save_scalars(summary_writer, 'train_dispnet', scalar_outputs_dispnet, global_step)
                ...
                if (global_step) % args.save_freq == 0:
                    ...
                    logger.info(f'Step {global_step} train dispnet: {total_err_metric_psmnet}')
        gc.collect()
...
def train_sample(sample, dispnet_model, dispnet_optimizer, isTrain=True):
    ...
    img_output_reproj = {
        'sim_reprojection': {
            'target': img_L_gray_scale, 'warped': sim_ir_warped, 'pred_disp': sim_pred_disp, 'mask': sim_ir_reproj_mask
        },
        'real_reprojection': {
            'target': img_real_L_gray_scale, 'warped': real_ir_warped, 'pred_disp': real_pred_disp, 'mask': real_ir_reproj_mask
        }
    }
...
if __name__ == '__main__':
    ...
    dispnet_optimizer = torch.optim.Adam(dispnet_model.parameters(), lr=cfg.SOLVER.LR_CASCADE, betas=(0.9, 0.999))
