...
class DispNet(torch.nn.Module):
    ...
    # decoder part start
        ...
        self.upsample_flow1to0=torch.nn.ConvTranspose2d(in_channels=1,out_channels=1,kernel_size=4,stride=2,padding=1)
...
def dispnet_loss(disp_ests, disp_gt, mask):
    scale = [0, 1, 2, 3, 4, 5, 6]
    ...
    for disp_est, weight, s in zip(disp_ests, weights, scale):
        if s != 0:
            ...
            dgt = disp_gt
            m = mask
        all_losses.append(weight * F.smooth_l1_loss(disp_est[m], dgt[m], size_average=True, reduction='mean'))
